--[[
	1. added auto farm mastery
	2. added auto pull lever
	3. added auto awake fruit
	4. added item finder fully
	5. fixed auto dough awaken ( not have check remote )
--]]
...
LPH_JIT_MAX(function()
	...
	function Bartilo_Quest()		
		...
		if not Remote_Check_Bartilo.KilledSpring then
			local Quest_gui = localplayer.PlayerGui.Main.Quest
...
Second_Sea_Function = LPH_JIT_MAX(function()
	...
	elseif getgenv().KaitunConfig["Sword"]["World 2"]["Dragon Trident"] and getgenv().KaitunConfig["Setting"]["Hop Server"] and Level >= 1500 and not Check_Monster("Tide Keeper") and not Has_Dragon_Trident then
		Server_Hop("Finding Tide Keeper")
		return true
	...
	elseif getgenv().KaitunConfig["Sword"]["World 2"]["True Triple Katana"] and Level >= 1500 and not Has_TTK then
		Get_TTK()
		return true
	elseif Level >= 1500 and Check_Item_Condition("Sword", "World 2", "Rengoku", Has_Rengoku) and Check_Item_Condition("Sword", "World 2", "Tide Keeper", Has_Dragon_Trident) and Check_Item_Condition("Sword", "World 2", "Gravity Cane", Has_Gravity_Cane) and Check_Item_Condition("Gun", "World 2", "Kabucha", Has_Kabucha) and Check_Item_Condition("Sword", "World 2", "True Triple Katana", Has_TTK) then
		Travel_To_Third_Sea()
		return true
	end
...
localplayer.CharacterAdded:Connect(function(Character)
	...
	humanoid.StateChanged:Connect(function(oldState, newState)
		if newState == Enum.HumanoidStateType.Seated then
			print("💺 Human-sit is true")

			task.wait(0.1)

			if humanoid.Sit then
				print("⚡ Human-sit is false")
				humanoid.Sit = false
			end
		end
	end)
